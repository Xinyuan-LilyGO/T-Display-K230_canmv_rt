...
DISPLAY_WIDTH = 568
DISPLAY_HEIGHT = 1232
custom_display_width=992
custom_display_height=560
app_exit=0
sensor = None
counter=1
#msg_transfer=""
...
def lora_test_demo():
    ...
    if soft_spi is None:
        soft_spi=SoftSPI(0,baudrate=2000000, polarity=0, phase=0,bits=8,miso=17, mosi=16, sck=15)
        #soft_spi=SPI(1,baudrate=2000000, polarity=0, phase=0, bits=8)
    #modem_lora=SX1276(spi=soft_spi,cs=Pin(6,Pin.OUT),dio0=None,dio1=None,reset=Pin(5,Pin.OUT),lora_cfg=lora_cfg_data)
    modem_lora=SX1276(spi=soft_spi,cs=Pin(14,Pin.OUT),dio0=None,dio1=None,reset=Pin(5,Pin.OUT),lora_cfg=lora_cfg_data)
...
def lora_test():
    ...
    lora_init()
    lora_test_demo()
def network_wlan_demo():
    global network_label
    if network_label==None:
        network_label = lv.label(lv.scr_act())
        # Position the main label
        network_label.align(lv.ALIGN.TOP_MID, 0, 100)
    network_label.set_text("")
    str_content="";
    sta=network.WLAN(0)
    #sta连接ap
    sta.connect("c3","12345678")
    #查看sta状态
    if True==sta.status() and sta.isconnected() and sta.ifconfig()[0] != '0.0.0.0' :
        str_content+=f"ssid:c3 connect OK\n"
        str_content+=f"ip:{sta.ifconfig()}\n"
        network_label.set_text(str_content)
    else:
        str_content+=f"ssid:c3 connect fail\n"
    network_label.set_text(str_content)
    str_content=""
    #断开连接
    sta.disconnect()
    ...
    print(sta.status())
    '''
def network_wlan_ui_init():
    print("network_wlan_ui_init")
def music_test():
    print("music test")
...
def hdmi_test():
    print("hdmi test")
def network_wlan_test():
    print("network waln")
    #network_wlan_ui_init()
    network_wlan_demo()
...
def camera_show_object_detect():
    ...
    rgb888p_size = [custom_display_width, custom_display_height]
    display_size=[custom_display_width,custom_display_height]
    # 模型路径
    ...
    try:
        while True:
            ...
            with ScopedTiming("total",1):
                ...
                Display.show_image(img_frame1,x=0,y=0, alpha = 128,flag=K_ROTATION_90)
                img = sensor.snapshot(chn = CAM_CHN_ID_2)
                # 推理当前帧
                res=ob_det.run(img.to_numpy_ref())
                # 绘制结果到PipeLine的osd图像
                osd_img = image.Image(custom_display_width, custom_display_height, image.ARGB8888)
                ob_det.draw_result(osd_img,res)
                # 显示当前的绘制结果
                if(try_count>800):
                    osd_img.clear()
                    Display.show_image(osd_img,x=0,y=0, alpha = 128,layer = Display.LAYER_OSD1,flag=K_ROTATION_90)
                    gc.collect()
                    break
                Display.show_image(osd_img,x=0,y=0, alpha = 128,layer = Display.LAYER_OSD1,flag=K_ROTATION_90)
                gc.collect()
...
def sensor_init():
    ...
    sensor.set_framesize(width = custom_display_width, height = custom_display_height)
    # set chn0 output format
    sensor.set_pixformat(Sensor.YUV420SP)

    sensor.set_framesize(width = custom_display_width, height = custom_display_height, chn = CAM_CHN_ID_1)
    sensor.set_pixformat(Sensor.RGB888, chn = CAM_CHN_ID_1)
    sensor.set_framesize(width = custom_display_width, height = custom_display_height, chn = CAM_CHN_ID_2)
...
def user_gui_init():
    ...
    try:
        with open('/sdcard/examples/utils/desktop.png','rb') as f:
        #with open('/sdcard/examples/utils/animimg003.png','rb') as f:
            png_data = f.read()
    except:
        print("Could not find desktop.png")
        sys.exit()

    img_desktop = lv.img_dsc_t({
    ...
    style.set_img_recolor_opa(lv.OPA.TRANSP)
    style.set_img_recolor(lv.color_black())
    '''
    img = lv.img(lv.scr_act())
    #img.add_style(style, 0)
    img.set_src(img_desktop)
    img.set_size(500, 800)
    img.center()
    '''
    a = lv.anim_t()
    a.init()
