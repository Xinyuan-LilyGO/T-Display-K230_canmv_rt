...
class Display:
    ...
    def init(cls, type = None, width = None, height = None, osd_num = 1, to_ide = False, flag = None, fps = None, quality = 90):
        ...
        if _type >= Display.VIRT:
            ...
            elif _type == Display.RM69A10:
                _width = width if width is not None else 568
                _height = height if height is not None else 1232
                #_flag = flag if flag is not None else Display.FLAG_ROTATION_90

                #cls._connector_is_rm69a10 = True

                if _width == 568 and _height == 1232:
                    #cls._ide_vo_wbc_flag = _flag
                    cls._connector_type = RM69A10_MIPI_2LAN_568X1232_60FPS
                else:
                    raise ValueError(f"RM69A10 unsupoort {_width}x{_height}")
