...
int main(int argc, char *argv[])
{
    ...
    while(app_run)
    {
        ...
        for (size_t i = 0, j = 0; i < boxes.size(); i += 1)
        {
            ...
            vo_frame.line_y_start = (((uint32_t)boxes[i].x2) * ISP_CHN0_WIDTH / ISP_CHN1_WIDTH);
            vo_frame.line_x_end = ((uint32_t)boxes[i].y2) * ISP_CHN0_HEIGHT / ISP_CHN1_HEIGHT;
            vo_frame.line_y_end =(((uint32_t)boxes[i].x1) * ISP_CHN0_WIDTH / ISP_CHN1_WIDTH);*/
