...
#include "Setting.hpp"
//#include "app_sntp.h"
#include "../../../../../network/network_rt_opt.h"
...
typedef enum {
    WIFI_EVENT_CONNECTED = (1<<0),
    WIFI_EVENT_INIT_DONE = (1<<1),
    WIFI_EVENT_UI_INIT_DONE = (1<<2),
    WIFI_EVENT_SCANING = (1<<3)
} wifi_event_id_t;
...
bool AppSettings::run(void)
{
    ...
    ui_setting_init();
     network_rt_init();
     printf("network_rt_init\n");
    // Get MAC
     lan_get_mac(base_mac_addr);
...
void AppSettings::extraUiInit(void)
{
    ...
    for(int i = 0; i < SCAN_LIST_SIZE; i++) {
        ...
        lv_obj_set_style_bg_color(panel_wifi_btn[i], lv_color_hex(0xCBCBCB), LV_PART_MAIN);
        lv_obj_set_style_bg_opa(panel_wifi_btn[i], 255, LV_PART_MAIN);
        lv_obj_set_style_border_color(panel_wifi_btn[i], lv_color_hex(0xFFFFFF), LV_PART_MAIN);
        lv_obj_set_style_border_opa(panel_wifi_btn[i], 255, LV_PART_MAIN);
    ...
    lv_obj_add_event_cb(ui_KeyboardScreenSettingVerification, onKeyboardScreenSettingVerificationReadyCallback, LV_EVENT_READY, this);
    //lv_keyboard_set_mode(ui_KeyboardScreenSettingVerification, LV_KEYBOARD_MODE_NUMBER);
    ...
    /* Bluetooth */
    lv_obj_add_flag(ui_PanelSettingMainContainerItem2, LV_OBJ_FLAG_HIDDEN);//hide bluetooth func
    ...
    lv_label_set_text(ui_LabelPanelPanelScreenSettingAbout2, "k230-Lilygo-Board");
...
void AppSettings::startWifiScan(void)
{
    ...
    for(int32_t i = 0; i < show_wlan_num; i++) {
        ...
        memcpy(ssid_buf,item_wlan_info.ssid.val,item_wlan_info.ssid.len);
          ...
          /*
          lv_label_set_text_fmt(label_wifi_ssid[i], "%s", (const char*)ssid_buf);
          lv_obj_clear_flag(label_wifi_ssid[i], LV_OBJ_FLAG_HIDDEN);
        ...
        lv_img_set_src(wifi_image[i], &img_wifisignal_good);
        lv_obj_clear_flag(panel_wifi_btn[i], LV_OBJ_FLAG_HIDDEN);
        */
        lv_obj_clear_flag(panel_wifi_btn[i], LV_OBJ_FLAG_HIDDEN);
        if(item_wlan_info.security != SECURITY_OPEN) {
            psk_flag=true;
        }
        else
        {
        ...
        if(item_wlan_info.rssi > -100 && item_wlan_info.rssi <= -80) {
            _wifi_signal_strength_level = WIFI_SIGNAL_STRENGTH_WEAK;
        } else if(item_wlan_info.rssi > -80 && item_wlan_info.rssi <= -60) {
            _wifi_signal_strength_level = WIFI_SIGNAL_STRENGTH_MODERATE;
        } else if(item_wlan_info.rssi > -60) {
        ...
        initWifiListButton(label_wifi_ssid[i], img_img_wifi_lock[i], wifi_image[i], wifi_connect[i],
                                item_wlan_info.ssid.val, psk_flag, _wifi_signal_strength_level);
                                     ...
                                     lv_obj_t *lv_wifi_connect, uint8_t* ssid, bool psk, WifiSignalStrengthLevel_t signal_strength)
{
    ...
    if (strcmp((const char*)ssid, (const char*)st_wifi_ssid) == 0) {
        lv_obj_clear_flag(lv_wifi_connect, LV_OBJ_FLAG_HIDDEN);
          if(signal_strength == WIFI_SIGNAL_STRENGTH_WEAK) {
                this->status_bar->setWifiIconState(1);
            } else if(signal_strength == WIFI_SIGNAL_STRENGTH_MODERATE) {
                this->status_bar->setWifiIconState(2);
            } else if (signal_strength == WIFI_SIGNAL_STRENGTH_GOOD) {
                this->status_bar->setWifiIconState(3);
            }
    }
...
*/
void AppSettings::onKeyboardScreenSettingVerificationReadyCallback(lv_event_t *e)
{
    ...
    ESP_BROOKESIA_CHECK_NULL_GOTO(app, end, "Invalid app pointer");      
        //app->stopWifiScan();
      {  char* ssid=lv_label_get_text(ui_LabelScreenSettingVerificationSSID);
        char* key=(char*)lv_textarea_get_text(ui_TextAreaScreenSettingVerificationPassword);
        printf("ssid:%s\n",ssid);
        printf("key:%s\n",key);
        network_rt_wlan_connect(ssid,key);
        //network_rt_wlan_connect((char*)"c3",(char*)"12345678");
        //app->processWifiConnect(WIFI_CONNECT_RUNNING);
        int is_connected;
        network_rt_wlan_isconnected(&is_connected);
        if(is_connected==1)
        {  
           memcpy(st_wifi_ssid, ssid, strlen(ssid));                     
        //app->processWifiConnect(WIFI_CONNECT_SUCCESS);
        }
        else
        {
